...
function ShapChart({ shapValues }) {
  ...
  const data = Object.entries(shapValues)
    .map(([feature, value]) => ({ feature, value: Number(Number(value).toFixed(4)) }))
    ...
    <div style={{ marginTop: '1rem' }}>
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} layout="vertical" margin={{ left: 80 }}>
          ...
          <YAxis type="category" dataKey="feature" width={90} fontSize={11} />
...
export default function DiabetesPredictionResult({ prediction }) {
  ...
  return (
    <div className="card">
      ...
      <div style={{ fontSize: '0.85rem', color: '#718096', display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
        {prediction.record_id && <span>Record ID: {prediction.record_id}</span>}
